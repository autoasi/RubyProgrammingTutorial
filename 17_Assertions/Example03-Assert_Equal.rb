...
class TC001_Verify_Login_Functionality < Test::Unit::TestCase
    ...
    def testSuccessLogin
        ...
        assert_equal("User Logged on", "user logged on","Error - Expected result not equal to actual result")  # Result - Assertion failed
    end

    def testInvalidUsernameLogin
        puts "User could not be logged on as expected"
        assert_equal("User failed to logon", "User failed to logon", "Error - Expected result not equal to actual result")  # Result - Assertion passed
    end
